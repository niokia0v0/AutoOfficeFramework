...
def process_single_file(input_path):
    ...
    支持处理多页（Multi-sheet）Excel文件。
    """
    # 根据输入路径，构造输出路径
    base_name = os.path.basename(input_path)
    dir_name = os.path.dirname(input_path)
    output_filename = f"output_invoice_{base_name}"
    ...
    try:
        ...
        # 使用 openpyxl 加载工作簿，以便后续读取单元格格式（如颜色）
    ...
    # 删除默认创建的空白 Sheet，以便后续按顺序添加与源文件对应的 Sheet
    ...
    for sheet_name in sheet_names:
        ...
        # 读取当前 Sheet 的数据
        df = pd.read_excel(xls_file, sheet_name=sheet_name, header=0)
        try:
            sheet_ole = workbook_ole[sheet_name]
        except KeyError:
            print(f"     警告: 无法在样式对象中找到表 '{sheet_name}'，跳过样式读取。")
            sheet_ole = workbook_ole.active # Fallback

        # --- 步骤 2: 数据预处理和准备 ---
        # 删除'单号'列为空的行，这些行是无效数据
        # 检查是否存在'单号'列，防止空Sheet报错
        if '单号' not in df.columns:
            print(f"     跳过: 表 '{sheet_name}' 缺少关键列'单号'或为空表。")
            continue
        ...
        for index, row in df.iterrows():
            # 定位到原始Excel文件中对应的单元格
            try:
                # 注意：这里假设客户名称列位置固定，如果Sheet结构不同可能会有问题
                customer_col_idx = df.columns.get_loc('客户名称') + 1
                customer_cell = sheet_ole.cell(row=index + 2, column=customer_col_idx)
        ...
        # --- 步骤 3: 创建输出 Sheet 和样式 ---
        sheet_out = wb_out.create_sheet(title=sheet_name)
        ...
        for index, source_row in df.iterrows():
            ...
            if num_batches > 1 and num_specs > 1 and num_batches != num_specs:
                print(f"     -> 警告: 源表 '{sheet_name}' 第 {index+2} 行数据批号({num_batches})与规格({num_specs})不匹配。客户: {customer_name}")
    ...
    # --- 保存文件 ---
    try:
        # 如果所有Sheet都因为无数据被跳过，wb_out可能是空的（除了被删除的默认页），为了避免保存空文件，这里不作特殊处理，允许保存空壳
        wb_out.save(output_path)
        print(f"  -> 成功! 文件已保存至:\n     {output_path}\n")
    except PermissionError:
        print(f"  -> 错误: 权限不足。文件 '{base_name}' 的输出文件可能正被Excel或其他程序占用。")
        print("     请关闭相关程序后重试。\n")

def main():
    """
    程序的主入口。负责处理命令行参数（拖拽的文件），并为每个文件调用处理函数。
    """
    # 从命令行参数中获取文件列表
    files_to_process = sys.argv[1:]

    # 如果没有文件被拖拽（即直接双击exe），则显示用法说明
    if not files_to_process:
        print("发票记录转换工具")
        print("用法: 请将一个或多个xlsx格式的发票记录文件拖拽到本程序的图标上。")
        input("\n按回车键退出...")
        return
    ...
    for file_path in files_to_process:
        ...
        if not os.path.exists(file_path) or not file_path.lower().endswith('.xlsx'):
            print(f"跳过: '{os.path.basename(file_path)}' 不是一个有效的xlsx文件。")
            continue
        
        # 使用try-except块来捕获处理单个文件时可能发生的任何错误
        try:
            # 调用核心处理函数
            process_single_file(file_path)
        except Exception as e:
            # 如果发生未知错误，打印错误信息，但程序会继续处理下一个文件
            print(f"\n处理 '{os.path.basename(file_path)}' 时发生未知错误: {e}\n")

    # 所有文件处理完毕后，等待用户按回车键退出
